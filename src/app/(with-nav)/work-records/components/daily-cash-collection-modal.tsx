...
export function DailyCashCollectionModal({
  ...
  const targetDate = useMemo(
    () => format(subDays(baseDate, 1), "yyyy-MM-dd"),
    [baseDate],
  );
  const { data, isLoading, error } = useDailyCashCollection(targetDate, {
    enabled: open,
  });
  ...
  const pendingGrandTotal = useMemo(
    () => (data?.rows ?? []).reduce((acc, row) => acc + row.pendingAmount, 0),
    [data?.rows],
  );
  const combinedGrandTotal = (data?.grandTotal ?? 0) + pendingGrandTotal;
  const hasAnyPending = pendingGrandTotal > 0;
            ...
            <>
              {hasAnyPending && (
                <p className="px-1 pb-2 text-xs text-gray-500">
                  금액 옆 ( )는 수금 확인 요청 중인 금액이며, 좌측 합산에
                  포함되어 있습니다.
                </p>
              )}
              <div className="overflow-hidden rounded-md border border-gray-200">
                ...
                <div className="divide-y divide-gray-100">
                  {data.rows.map((row) => {
                    const combined = row.totalAmount + row.pendingAmount;
                    const combinedCount = row.recordCount + row.pendingCount;
                    return (
                      <div
                        key={row.userId}
                      ...
                      >
                        ...
                        <div className="text-right text-sm font-semibold text-gray-900">
                          {combined.toLocaleString("ko-KR")}원
                          {row.pendingAmount > 0 && (
                            <span className="ml-1 font-normal text-gray-500">
                              ({row.pendingAmount.toLocaleString("ko-KR")}원)
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <div className="mt-4 flex items-center justify-between border-t border-gray-200 px-4 py-3">
                ...
                <span className="text-base font-bold text-gray-900">
                  {combinedGrandTotal.toLocaleString("ko-KR")}원
                  {pendingGrandTotal > 0 && (
                    <span className="ml-1 text-sm font-normal text-gray-500">
                      ({pendingGrandTotal.toLocaleString("ko-KR")}원)
                    </span>
                  )}
                </span>
              </div>
