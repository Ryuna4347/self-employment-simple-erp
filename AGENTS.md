...
├── register/            # 회원가입
├── api/                 # API 라우트
├── maintenance/         # 점검 페이지 (MAINTENANCE_MODE 활성 시 rewrite 대상)
...
│   ├── auth-guard.ts        # 인증 검증 유틸 (requireAuth/Admin/WriteAccess/AdminRead)
│   ├── collection-utils.ts  # 수금 관련 계산
│   ├── sales-utils.ts       # 매출 원금(salesAmount) 규칙, 이월 항목 판별, 집계 기준일
│   ├── date-utils.ts        # 날짜 포매팅 (KST)
│   ├── excel/               # 엑셀 내보내기
│   │   ├── monthly-report.ts
│   │   ├── utils.ts
│   │   └── types.ts
...
│   └── daily-cash-collection/route.ts # GET 일별 직원 현금 수금 집계 (ADMIN)
├── collection-requests/
...
- **Store**: 매장 (name, address, managerName, PaymentType, coordinates, receiptType, note, assignedUserId, isDeleted, deletedAt)
- **StoreItem**: 매장 기본 품목 (storeId, name, amount, quantity)
- **WorkRecord**: 방문 기록 (date, storeId, userId, collectionStatus, imageUrl, snapshots, sortOrder, collectedAt, collectedByUserId)
- **RecordItem**: 거래 상세 (workRecordId, name, amount, salesAmount, quantity)
  - `amount`: 현재 잔액(수금 장부). 수금 처리 시 0, 이월 수금 시 마지막 건으로 이동
  - `salesAmount`: 매출 원금. 등록 시점 amount와 동일, 수금 처리로 변하지 않음. 이월 수금 항목은 0. 백필 없이 `SALES_AMOUNT_CUTOVER_DATE`(`src/lib/sales-utils.ts`)부터 수집
...
- **CollectionRequest**: 수금 확인 요청 (storeId, storeNameSnapshot, requesterId, status, reviewerId, reviewedAt)
...
### 실험용 Clone 모델 (FK 없는 self-contained 복제 테이블)
- **StoreClone / WorkRecordClone / RecordItemClone**: 원본 데이터를 안전하게 복제해 실험/수정 작업에 사용. 원본 ID는 `sourceId` 컬럼으로만 추적하고 FK 관계 없음. 운영 화면/API에서는 참조하지 않음.
...
- **점검 모드**: `MAINTENANCE_MODE=true|1|on` 환경변수로 토글. 미들웨어가 모든 페이지는 `/maintenance` 로 rewrite, 모든 API(`/api/auth`, `/api/cron` 포함)는 503 JSON 응답으로 차단한다. 해제는 변수 제거 또는 `false`.
...
| 2026-09-03 | 근무기록 수정(PUT) 시 기존 품목의 salesAmount 보존 (`toRecordItemDataPreservingSales`: 품목 단위 매칭 + 기록 단위 합계 보존) | lib/sales-utils, work-records/[id] API, work-records AGENTS.md | 수정은 품목 삭제 후 재생성 구조라, 수금 처리로 amount가 0이 된 기록을 어드민이 고치면 매출 원금이 0으로 덮어써지는 회귀 방지. 최상위 품목 1개에 기록 합계를 적는 현장 관행(품목명 변경/삭제 시 개별 매칭 실패)을 기록 합계 보존으로 반영. 기준일은 백필 대신 배포 다음 날(09-04)로 설정 |
