...
│   ├── auth-guard.ts        # 인증 검증 유틸 (requireAuth/Admin/AdminRead/WriteAccess/Role + requireApiKey 외부 API)
...
│   ├── tax-invoice-utils.ts # 세금계산서 부가세 분리/포맷 유틸 (외부 API용)
│   ├── utils.ts             # 범용 유틸리티
...
├── tax-invoices/
│   ├── pending/route.ts               # GET 미발행 세금계산서 집계 (외부 발급기, X-API-Key)
│   └── result/route.ts                # PUT 발행 결과 보고 (외부 발급기, X-API-Key)
├── cron/
...
- **Store**: 매장 (name, address, managerName, PaymentType, kakaoPlaceId, latitude/longitude, receiptType, note, taxInvoiceEnabled, taxPartyId, assignedUserId, isDeleted, deletedAt)
- **TaxParty**: 사업자 마스터 (name, bizNo, representativeName, businessType, businessItem, taxInvoiceEmail, address, isDeleted, deletedAt)
- **StoreItem**: 매장 기본 품목 (storeId, name, amount, quantity)
- **WorkRecord**: 방문 기록 (date, storeId, userId, collectionStatus, imageUrl, note, snapshots, sortOrder, collectedAt, collectedByUserId)
...
- **CollectionRequest**: 수금 확인 요청 (storeId, storeNameSnapshot, requesterId, status, reviewerId, reviewedAt, note)
...
- **TaxInvoiceResult**: 세금계산서 발행 결과 (storeId, year, month, idempotencyKey, status, mode, submittedAt, retryCount, errorReason)
...
- **TaxInvoiceStatus**: SUBMITTED, SKIPPED, FAILED
...
- 한국어 도메인 문서(루트 1개 + 도메인별 AGENTS.md 15개 = 16개)를 분석하는 컨텍스트 수집 절차가 별도로 정의됨
